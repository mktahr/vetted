...
### Piece A (role-attributed skills) — CLOSED 2026-08-02: no source provides role-attribution. DO NOT REINVESTIGATE.
- **Decision:** Piece A is cancelled as originally scoped. **Skills are profile-level-only, permanently** — no available data source ties a skill to a specific role/company/date. Piece B (shipped, PR #17) already captures exactly this profile-level shape and stores it in `people.skills_matched`. There is no per-role skills layer to build. **Migration 099 and the "no-wipe ingest redesign" are CANCELLED** — they existed solely to store per-role declared skills, and there is no data to populate them.
- **WHY (proven against real data 2026-07-31/08-02, not docs — so this is never reinvestigated):**
  - **Crust enrich (~99% of our profiles):** skills arrive ONLY as `skills.professional_network_skills` = a flat `string[]` of bare names. Verified against real stored `network_enriched_profiles` blobs. Separately scanned **100 positions across 10 real profiles**: the union of all position-object keys contains NO skill field (0/100). Skills and experience are disjoint branches of the payload — no `position_id` cross-reference, nothing links a skill to a role.
  - **Crust person-search path:** returns NO skills at all (`crust-v2.ts` sets `skills_tags: null` — documented TODO). Only the enrich path carries skills.
  - **LinkedIn voyager scraper (the <1% fallback):** `dash/profileSkills` returns 200 but a flat list — each skill is `{name, entityUrn: urn:li:fsd_skill:(profileId, index)}`, just a name + ordinal, `positionUrnCount: 0`. The legacy association endpoints (`/profiles/{vanity}/skills`, `/skillCategory`) are **410 Gone**. The per-role attributions shown on LinkedIn's `/details/skills/` page ("Systems Design · used at Ponto") are assembled by the SDUI display layer at render time (binary octet-stream component tree), not exposed as structured data by any API.
