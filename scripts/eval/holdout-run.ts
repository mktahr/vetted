// scripts/eval/holdout-run.ts
// ONE-SHOT generalization run over the LOCKED holdout set (from eval-sets.json).
// Same harness as tuning-run.ts (honest buckets, production validation-retry, both
// deterministic guards) but reads sets.holdout, writes holdout-report.md, and appends
// to holdout-run.log (NOT tuning-trend.log). Run ONCE against the frozen prompt+vocab;
// re-running against a tuned prompt invalidates the holdout's purpose.
// Run: tsx scripts/eval/holdout-run.ts
...
async function main(){
  ...
  const tuningIds = new Set<string>(sets.holdout)
  ...
  const L:string[]=[]
  L.push(`# HOLDOUT one-shot — Haiku (${sets.holdout.length} candidates) · prompt ${PROMPT_VERSION}`)
  L.push(``)
  ...
  writeFileSync('reference/eval/holdout-report.md', L.join('\n'))
  const cost = (inTok/1e6)*IN_PER_M + (outTok/1e6)*OUT_PER_M
  const pctStr = comparable?(100*counts.agree/comparable).toFixed(1):'n/a'
  console.log(`\nWrote reference/eval/holdout-report.md — agree ${counts.agree} / disagree ${counts.disagree} / both-unknown ${counts.both_unknown} / error ${counts.error} / no-ref ${counts.no_ref}`)
  ...
  appendFileSync('reference/eval/holdout-run.log', `${new Date().toISOString()} ${PROMPT_VERSION}  comparable=${pctStr}% (n=${comparable})  agree=${counts.agree} disagree=${counts.disagree} both_unknown=${counts.both_unknown} error=${counts.error}  cost=$${cost.toFixed(4)}\n`)
}
