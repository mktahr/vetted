...
| **Skills** | `person_experiences.skills_inferred[]` (live, classifier); aggregated `skills[]` view in sub-PR 4 | `skills_dictionary` — 192 active on BOTH dev + prod (synced at merge 2026-07-08 via `/reference/skills/*.csv`). Flat; `primary_specialty` = soft multi-domain HINT, never ownership |
...
### Vocab state — FROZEN with the prompt; PROD == DEV (byte-identical, verified 2026-07-08)
Both DBs: 17 active functions / **150 active specialties** (090: +computer_vision/nlp/sre/devops/platform; 091: +robotics_software_engineering reactivated-guarded + thermal/reliability parent-widenings; 093: +powertrain_engineering [mechanical, electrical]; 096: 4 prod-only drift rows deactivated — blockchain/game/qa_testing/robotics) / **192 active skills**. Parity is name + parent-array + role-map level — the vocab hash `33c400c8` is identical on both, which is what keeps the frozen `cls-2026-07-08a` contract valid on prod. `sync-reference.mjs` has `--dev` (targets dev; default prod) + a hard gate validating every skills hint against the TARGET DB's active specialties (fails loud, also in --dry-run). ⚠ Any future vocab change must go to BOTH DBs (dev-first) or the hash forks.
