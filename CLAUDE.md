...
Classifier tuning batch (Pavlo→backend+data_pipeline from About on sparse roles; Michael→fullstack from career; Joanne→don't invent niche specialties) → cheap tuning re-run → re-populate. Holdout one-shot run. Full-corpus POOL run. Hardening-before-merge (BUGS.md). Prod taxonomy 085–087 + person-data cascade + full re-score. PR + merge. `mission_systems_engineering` keep-or-collapse decision. `employment_type`/full-time capture cleanup.
