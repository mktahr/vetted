...
Decoupled, spend-capped Haiku 4.5 classifier that reads each work experience and infers the five axes (writes `person_experiences.function_inferred/specialty_inferred/skills_inferred/title_normalized_inferred`). NOT merged. Prompt frozen at `PROMPT_VERSION='cls-2026-07-02a'` (in `lib/candidates/classifier/config.ts`). Vocab-only changes do NOT bump PROMPT_VERSION — the vocab hash (now including specialty parents) carries that provenance in `classifier_version`.

### Engine (`lib/candidates/classifier/`)
- `config.ts` (model `claude-haiku-4-5`, PROMPT_VERSION, LEASE_MINUTES, MAX_FAILURES, MAX_VALIDATION_RETRIES=1, spend cap), `prompt.ts` (buildSystemPrompt loads active vocab dynamically; governing principle + Rules 1–8; Rule 7 = scoped headline/summary context; candidate text delimited as untrusted), `claude.ts` (`callClassifier`, never throws — returns `{output,error,tokens}`), `validate.ts` (strict vocab membership + EXACT experience coverage + **two REJECT-then-REPAIR guards**: parent-function guard — specialty whose `parent_function` excludes every assigned function — and unknown-skill guard; both ERROR on attempt 0 so the retry can fix either axis, then STRIP-and-commit on the final attempt, recorded in `result.repairs`, so one stray value never fails a candidate; **known gap: unknown SPECIALTY names are NOT yet repaired on the final attempt** — extension queued), `index.ts` (`loadActiveVocab` — also loads the specialty→`parent_function[]` map, parents participate in the vocab hash; `classifyCandidate` = claim→reserve-per-call→Haiku+1 validation-retry→fenced `commit_classification`, `classifyPending` = batch that HALTS after 3 consecutive infra/API discards).
- Lifecycle is generation-fenced + app-layer lease; failure semantics: only classifier-unusable results burn budget (infra/cap/contention discard).
...
### Prompt rules (cls-2026-07-02a)
Governing principle IS-IT (specialty) vs TOUCHED-IT (skill). Function values come from the function list ONLY (a specialty like `ai_engineering` is NOT a function). Skills come from the skills list ONLY. Don't-abstain on a clearly-engineering title. AI/ML build-vs-use: explicit ML title → `ml_engineering`; vague "AI" → `software_engineering[ai_engineering]` unless model-building. Non-eng + skilled TRADES (machinist/CNC/technician) → unknown. Embedded → `firmware_engineering[embedded_engineering]`. Propulsion → `aerospace_engineering[propulsion_engineering]`. `systems_engineering` is its OWN function (route by work). Rule 6 leadership: infer discipline from the role's org (current-org primary), weak career fallback, don't bleed an old discipline. Rule 7 candidate context: headline/summary SUPPLEMENT only sparse roles; headline→current role; concrete occupational summary language ("backend development", "built data pipelines") IS evidence for sparse roles (02a — Pavlo fix); present-tense claims don't describe old roles; generic fluff isn't evidence; CAREER FALLBACK — sparse engineering role inherits the career's STABLE consistent specialty (02a — Michael fix). Specialty EVIDENCE BAR: never guess a niche specialty; empty specialty is correct when evidence is thin (02a — Joanne fix). PARENT CONSISTENCY stated in-prompt (backs the validator guard). TITLE-KEYWORD TRAP: "<X> Software Engineer" (Mission/Systems/Platform) is a SOFTWARE engineer — employer industry / "Mission" buzzword doesn't pull to aerospace (02a — Aadhya fix; names `mission_systems_engineering` as the anti-pattern).

### Eval harness (`scripts/eval/`, artifacts in `reference/eval/` — GITIGNORED PII)
Candidate-split (never by experience): `build-eval-sets.ts` → `eval-sets.json` (TUNING 23 / HOLDOUT 23 locked / POOL 67, stratified). `freeze-fixture.ts` → `fixture-inputs.json` (129 cands / 996 exp, frozen from prod). `tuning-run.ts` (Haiku over tuning set WITH context + production validation-retry; honest buckets — never counts unknown==unknown or errors as agreement; Opus reference from `_draft-rows.json`; appends `tuning-trend.log`). `test-founding-regex.ts` (26 cases). `populate-preview.ts` (writes `_inferred_preview` on prod). Cost estimate BEFORE every run; no auto-reruns; populate estimates carry a ±30% band (retry volume scales with vocab size — $1.82 actual vs $1.45 est on 2026-07-06). Tuning trend: 79.3→89.1→85.2→88.2–89.0% stable (cls-2026-07-01d) → **91.2% error=0 (cls-2026-07-02a + 188-skill vocab + guards)**. ⚠ trend-log lines within cls-2026-07-02a ran against DIFFERENT vocab sizes (4→187→188 skills) — compare within a vocab, not across. `tuning-run.ts` also emits GUARD-REPAIRED lines + an out-of-vocab-skills aggregate (attempt-0 rejections + final-attempt strips, per candidate) as the ongoing vocab-gap signal.

### In-app preview
Migration 089 `_inferred_preview` columns; `app/profile/[id]/page.tsx` (per-role card) + `app/components/ProfileDrawer.tsx` (header function/specialty + LinkedIn icon + work-history cards) + `app/components/ProfileTable.tsx` (Function + Specialty columns) read them via the shared `lib/classification/current-role.ts` (`currentRoleClassification` — primary-current role matching ingest order, career fallback for novelty/sparse titles; `formatAxisLabel` = Title Case + acronyms). The orange "AI CLASSIFICATION (PREVIEW)" label is temporary scaffolding to restyle at launch.
...
### Vocab state (classifier reads DEV)
DEV: 17 active functions / **148 active specialties** (migration 090 added computer_vision/nlp/sre/devops/platform — fixed the prompt-vocab drift where Rule 5 instructed sre/devops/platform_engineering that didn't exist) / **188 active skills** (Matt's full dictionary via `/reference/skills/*.csv`, flat with soft multi-domain `primary_specialty` HINTS — never ownership). `sync-reference.mjs` gained `--dev` (targets dev project; default prod) + a hard gate validating every skills hint against the TARGET DB's active specialties (fails loud, also in --dry-run).

### Not done yet (next session)
Matt verifies the re-populated preview in-app (checklist + deep links in SESSION_HANDOFF). Extend final-attempt repair to unknown SPECIALTY names + ~$0.04 targeted re-run for the 2 stale val-fail candidates (Jessica Henson, Thomas A.). Pending Matt calls: parent-widening (thermal+aero, reliability+electrical, strip-review cases), CAD/Simulation skill rows, Aadhya evidence-bar. Then: holdout one-shot run → full-corpus POOL run → hardening-before-merge (BUGS.md) → prod taxonomy 085–087+090 + person-data cascade + full re-score → PR + merge. `employment_type`/full-time capture cleanup (separate legacy batch).
