...
| **Skills** | `person_experiences.skills[]` (sub-PR 4) | `skills_dictionary` — 188 active on DEV (full dictionary landed 2026-07-06 via `/reference/skills/*.csv`; prod still has the old 14 until merge). Flat; `primary_specialty` = soft multi-domain HINT, never ownership |
...
## Five-Axis Classifier (sub-PR 3) — LLM ingest inference (branch `five-axis-subpr3-classify`, IN PROGRESS)

Decoupled, spend-capped Haiku 4.5 classifier that reads each work experience and infers the five axes (writes `person_experiences.function_inferred/specialty_inferred/skills_inferred/title_normalized_inferred` + code-computed `specialty_inherited`). NOT merged. Prompt FROZEN at `PROMPT_VERSION='cls-2026-07-08a'` (in `lib/candidates/classifier/config.ts`) — eval phase complete (tuning 89 / POOL 85.9 / holdout 82.1, error=0 everywhere); next-version rule changes queue in `lib/candidates/classifier/next-prompt-queue.md`, NOT the prompt. Vocab-only changes do NOT bump PROMPT_VERSION — the vocab hash (including specialty parents) carries that provenance in `classifier_version`.
...
### In-app preview
Migration 089 `_inferred_preview` + 092 `specialty_inherited_preview` columns; `app/profile/[id]/page.tsx` (per-role card) + `app/components/ProfileDrawer.tsx` (header function/specialty + LinkedIn icon + work-history cards) + `app/components/ProfileTable.tsx` (Function + Specialty columns) read them via the shared `lib/classification/current-role.ts` (`currentRoleClassification` — primary-current role matching ingest order, career fallback for novelty/sparse titles; returns `{fn, specs, inheritedSpecs}` — inherited render muted/italic + ⓘ everywhere; `formatAxisLabel` = Title Case + acronyms). The orange "AI CLASSIFICATION (PREVIEW)" label is temporary scaffolding to restyle at launch. Preview verified per-candidate 2026-07-08: Joanne (robotics/robotics_software), Michael (inherited [fullstack, backend]), Pavlo (recent roles inherit [backend, distributed]), Aadhya (conservative-empty), SeJun (powertrain evidenced + inherited).

### Legacy title-resolution fixes (this session, applied to prod via targeted backfills)
Both `seniority_normalized` (`lib/normalize/seniority.ts` — both resolvers now scan the TITLE for leadership/level signals after exact-match fails) and `title_level` (`lib/normalize/title-level.ts::extractTitleLevel` — added a leadership fallback) had the same compound-title bug ("Robotics Software Engineering Manager" → IC / unleveled). Backfilled 257 seniority + 88 title_level values for the preview cohort. **Scores/buckets were NOT re-run — stale until the ship-time full re-score.**

### Vocab state (classifier reads DEV) — FROZEN with the prompt
DEV: 17 active functions / **150 active specialties** (090: +computer_vision/nlp/sre/devops/platform; 091: +robotics_software_engineering reactivated-guarded + thermal/reliability parent-widenings; 093: +powertrain_engineering [mechanical, electrical]) / **192 active skills** (188 + CAD, Simulation, COMSOL, CMake via `/reference/skills/*.csv`; flat with soft multi-domain `primary_specialty` HINTS — never ownership). `sync-reference.mjs` has `--dev` (targets dev project; default prod) + a hard gate validating every skills hint against the TARGET DB's active specialties (fails loud, also in --dry-run). PROD still has the old 14 skills + pre-085 taxonomy — syncs at merge.

### Not done yet — the MERGE ARC (all prod-touching, gated)
Eval + hardening phases COMPLETE (2026-07-08). Remaining: (1) prod taxonomy migrations 085/086/087/090/091/093 + person-data cascade (`specialty_normalized`/`function_normalized` reclassification mirroring 073) + prod skills sync + full re-score; (2) flip search/scoring to read `_inferred` + `specialty_inherited`; (3) PR + Vercel-preview-before-merge. After merge: real classifier run populates the real `_inferred` columns via `commit_classification`. Separate batches: `employment_type`/full-time capture cleanup; dup-experiences ingest race (BUGS.md); extension skills-section scrape + person-level alias matcher (BACKLOG).
...
- 093 — `powertrain_engineering` specialty (**DEV-ONLY; joins the taxonomy set at merge**). Parents `[mechanical_engineering, electrical_engineering]` (genuine EV-era cross). SeJun strip-review resolution: an all-powertrain career was inexpressible — the model reached for `motor_drives_engineering` (motor-CONTROL electronics, a different discipline), the parent guard correctly stripped it, the evidence bar correctly left empty. Correct strip, missing row. Dev: 150 active specialties.
