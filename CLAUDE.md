...
| **Skills** | `person_experiences.skills_inferred[]` (live, classifier); aggregated `skills[]` view in sub-PR 4 | `skills_dictionary` — **196 active** on BOTH dev + prod (192 synced at merge 2026-07-08 + 4 from the 07-09 tactical pass: Git, GitHub, Linux, Distributed Systems — moved the vocab hash to `83e9c32a`) via `/reference/skills/*.csv`. Flat; `primary_specialty` = soft multi-domain HINT, never ownership |
...
PostgreSQL does not support multi-value foreign keys natively. Same pattern as `companies.industries[]` (migration 031) and `companies.domain_tags[]`. **STRAIGHT ANSWER on where the taxonomy lives (verified 2026-07-05): functions and specialties are MIGRATIONS-ONLY (002/016/017/071/072/085–087/090) — no CSV, no sync.** `sync-reference.mjs` handles exactly three tables: `signal_dictionary`, `investor_tiers`, `skills_dictionary`. (An earlier version of this paragraph claimed sync-reference validates `parent_function` — it never did; that validation claim was stale.) What IS enforced at the app layer: the classifier's validator (`lib/candidates/classifier/validate.ts`) rejects/repairs function×specialty pairs whose `parent_function` excludes every assigned function, and `sync-reference.mjs` hard-validates skills' `primary_specialty` hints against the target DB's active specialties.
...
## Five-Axis Classifier (sub-PR 3) — **MERGED + LIVE** (PR [#16](https://github.com/mktahr/vetted/pull/16) → `4c93378`, 2026-07-08)

Decoupled, spend-capped Haiku 4.5 classifier that reads each work experience and infers the five axes (writes `person_experiences.function_inferred/specialty_inferred/skills_inferred/title_normalized_inferred` + code-computed `specialty_inherited`). **The merge arc completed 2026-07-08** (everything gated on Matt): prod taxonomy migrations 085–097 → prod vocab byte-identical to dev (150 specs / 17 fns / 192 skills, names + parents + role map); full classifier run 129/129 candidates / 996/996 experiences / 0 val-fails, uniform `cls-2026-07-08a/claude-haiku-4-5/33c400c8`; THE FLIP (search + scoring + display read `_inferred` ∪ `specialty_inherited`); post-merge full re-score (82 vetted / 47 needs_review). The **daily classify-pending cron is LIVE** (13:00 UTC, `vercel.json`, `CRON_SECRET` auth) — ingest re-queues via `bump_classification_generation`, so new candidates classify with ~24h latency (ingest hook queued in BUGS). Prompt FROZEN at `PROMPT_VERSION='cls-2026-07-08a'` (in `lib/candidates/classifier/config.ts`); next-version rule changes queue in `lib/candidates/classifier/next-prompt-queue.md`, NOT the prompt. Vocab-only changes do NOT bump PROMPT_VERSION — the vocab hash (including specialty parents) carries that provenance in `classifier_version`.

### The FLIP — what reads what (post-merge, live)
- **Search** ([app/components/ProfileTable.tsx](app/components/ProfileTable.tsx)): per-role `specialties` = `specialty_inferred ∪ specialty_inherited` ('unknown' dropped); `all_specialties` = union across roles ("ever" scope); `current_specs` = STRICT current-role classification ("currently" scope). Role pills expand via the 094-remapped `role_specialty_map`. Legacy `specialty_normalized`/`primary_specialty` are NO LONGER read by candidate search (they remain on ingest/network surfaces).
- **Scoring** ([lib/scoring/score-candidate.ts](lib/scoring/score-candidate.ts)): `functionName` = STRICT current-role inferred function ?? legacy `people.current_function_normalized` (fallback preserves the recruiting override + pending-candidate behavior). No people-level inferred column — the aggregate is sub-PR 4's job (Matt's call).
- **`lib/classification/current-role.ts` has TWO modes.** DISPLAY (default): falls back to the most-recent classified role (novelty/sparse current titles), and to the most-recent role when nothing is `is_current`. STRICT (`{ strictCurrent: true }`): genuinely-current roles only — REQUIRED for scoring + currently/previously predicates (Codex catches: the display fallback let a current recruiter score as software via an old role, and let an ended-only candidate's past role match "currently X"). Display-vs-filter divergence is deliberate + Matt-accepted: display summarizes, filters are semantic.
- **Manual edits** ([app/api/people/[id]/route.ts](app/api/people/[id]/route.ts)): PATCH bumps classification generation (re-classify next cron); the edited legacy values do NOT flow into the inferred axes (known gap, BUGS fast-follow).
- **⚠ Prod PostgREST is 14.1** (dev is 14.5): `PATCH + or=` logical filters are UNSAFE on prod — 42703 with named returning columns; with `select=*` the update EXECUTES but returns an empty representation. Conditional mutations go through RPCs (see migration 097 / claim). Parity upgrade queued in BUGS.

### DETERMINISTIC layers (2026-07-08 architecture — deterministic > AI extended to inheritance)
The LLM classifies ONLY what each role's own evidence supports; CODE does the rest. Both layers run in `classifyCandidate` after validation and ride `commit_classification` atomically (migration 092); the preview scripts call the same pure functions.
- **`lib/classification/career-fallback.ts::computeCareerFallback()`** — sparse role (engineering function, empty evidenced specialty, real non-student title) inherits the dominant career specialty into `specialty_inherited` (NEVER `specialty_inferred`, which stays pristine LLM output). Rules: FORWARD-ONLY in time (never inherits from later roles; `is_current` counts as "now"; undated non-current roles never inherit); parent-consistency vs the target's functions; sources = EVIDENCED roles only (no chains), excluding internships/student/advisory/contract, deduped by (title|company|start|end) against the dup-ingest bug; dominance = count → primary-position count → recency, cap 2 (≥2 sources, or the lone source); leadership titles get recent-primary-only (1 value). Search reads evidenced ∪ inherited; UI renders inherited muted/italic + ⓘ.
- **`lib/candidates/classifier/carve-out-guard.ts::enforceRoboticsCarveOutGuard()`** — Haiku leaks the robotics carve-out on employer names ("Software Engineer" @ "Rapid Robotics"); prompt iteration didn't reliably fix it. Deterministic reroute on the PURE leak signature ONLY: robotics fn + `robotics_software_engineering` as the ONLY specialty + no robot term in the title + empty description → software_engineering, specialty stripped. Mixed-evidence tuples untouched (Codex: rerouting could orphan a robotics-parented sibling). After any guard repair, the engine RE-VALIDATES strict before commit (failure → discard `guard_produced_invalid`, no budget burn).
- Fixture suites (run with tsx, no LLM): `scripts/eval/test-career-fallback.ts` (11 groups), `test-carve-out-guard.ts` (6), `test-validate.ts` (15), `test-founding-regex.ts` (26).

### Engine (`lib/candidates/classifier/`)
- `config.ts` (model `claude-haiku-4-5`, PROMPT_VERSION, LEASE_MINUTES, MAX_FAILURES, MAX_VALIDATION_RETRIES=1, spend cap), `prompt.ts` (buildSystemPrompt loads active vocab dynamically; governing principle + Rules 1–8; Rule 7 = scoped headline/summary context; candidate text delimited as untrusted), `claude.ts` (`callClassifier`, never throws — returns `{output,error,tokens}`), `validate.ts` (strict vocab membership + EXACT experience coverage + **REJECT-then-REPAIR guards**, all ERROR on attempt 0 then repair on the final attempt, recorded in `result.repairs`: parent-function mismatch (strip), unknown skill (strip), unknown SPECIALTY (strip — closed the gap that val-failed 2 candidates), `unknown` mixed with real functions (drop unknown), specialties on unknown-only (strip), empty `title_normalized_inferred` (tolerate as NULL)), `index.ts` (`loadActiveVocab` — also loads the specialty→`parent_function[]` map, parents participate in the vocab hash; `classifyCandidate` = **claim via `claim_classification` RPC (migration 097 — NOT an app-layer conditional UPDATE; prod PostgREST 14.1 mishandles `PATCH + or=`)**→reserve-per-call→Haiku+1 validation-retry→carve-out guard→career fallback→fenced `commit_classification`, `classifyPending` = batch that HALTS after 3 consecutive infra/API discards), `next-prompt-queue.md` (Matt-approved rules awaiting the next PROMPT_VERSION bump). Batch sizing: a batch of 50 real candidates can exceed Vercel's 300s `maxDuration` (one mid-flight candidate self-healed via lease expiry when it happened); ~20 is comfortable.
- Lifecycle is generation-fenced + app-layer lease; failure semantics: only classifier-unusable results burn budget (infra/cap/contention discard). Lease/run DB errors are LOGGED (hardening 2026-07-08) — a silent release/close failure can't hide an infra fault.
- **`classification_status` is the live queue key** — a preview must NEVER write it (Codex caught this). Ingest writes `network_connection`-safe; classifyPending selects pending/failed(<MAX)/expired.

### Prompt rules (cls-2026-07-08a)
Governing principle IS-IT (specialty) vs TOUCHED-IT (skill). Function values come from the function list ONLY (a specialty like `ai_engineering` is NOT a function). Skills come from the skills list ONLY. Don't-abstain on a clearly-engineering title. AI/ML build-vs-use: explicit ML title → `ml_engineering`; vague "AI" → `software_engineering[ai_engineering]` unless model-building. Non-eng + skilled TRADES (machinist/CNC/technician) → unknown. Embedded → `firmware_engineering[embedded_engineering]`. Propulsion → `aerospace_engineering[propulsion_engineering]`. `systems_engineering` is its OWN function (route by work). Rule 6 leadership: THIS role's own context only — recent-roles inheritance REMOVED (deterministic layer's job). Rule 7 candidate context: headline/summary SUPPLEMENT only sparse roles; headline→current role; concrete occupational summary language IS evidence for sparse roles; generic fluff ("Programmer | Engineer | Entrepreneur") is NOT evidence and a sparse role with only fluff context gets an EMPTY specialty (08a — Michael fix); CAREER FALLBACK REMOVED from the prompt — inheritance is code's job. Specialty EVIDENCE BAR: never guess a niche specialty; empty specialty is correct when evidence is thin. PARENT CONSISTENCY stated in-prompt (backs the validator guard). TITLE-KEYWORD TRAP: "<X> Software Engineer" (Mission/Systems/Platform) is a SOFTWARE engineer — with ONE exception, the ROBOTICS CARVE-OUT (08a — Joanne fix): a robotics modifier is a BUILD-target, not context → `robotics_engineering`, specialty = sharper robotics specialty when evidenced else `robotics_software_engineering`; keys on the TITLE or described work, NEVER the employer (backed by the deterministic guard).

### Eval harness (`scripts/eval/`, artifacts in `reference/eval/` — GITIGNORED PII)
Candidate-split (never by experience): `build-eval-sets.ts` → `eval-sets.json` (TUNING 23 / HOLDOUT 23 / POOL 67, stratified). `freeze-fixture.ts` → `fixture-inputs.json` (129 cands / 996 exp, frozen from prod). `tuning-run.ts` / `holdout-run.ts` / `pool-run.ts` (same harness over the respective set: Haiku WITH context + production validation-retry + BOTH deterministic guards; honest buckets — never counts unknown==unknown or errors as agreement; Opus reference from `_draft-rows.json`; per-set report + log files; inherited(code) lines shown for review). `populate-preview.ts` (writes `_inferred_preview` + `specialty_inherited_preview` on prod; `--ids=<uuid,...>` targeted mode for cheap per-candidate re-runs). Cost estimate BEFORE every run; no auto-reruns; populate estimates ±30%. **EVAL PHASE COMPLETE on frozen cls-2026-07-08a (2026-07-08): tuning 89.0–89.7 → HOLDOUT one-shot 82.1% (n=134) → POOL 85.9% (n=383), error=0 + 0 val-fails everywhere.** Holdout/POOL disagreement audits: dominated by STALE-Opus-reference artifacts (refs predate the new vocab/rules) + correct conservative abstentions; ~96% conformance to our own rules; real miss classes queued in `next-prompt-queue.md` (frontier-lab research titles ×6, embedded-vs-carve-out precedence, COBOL abstention). ⚠ the Opus reference is aging with every vocab/rule change — regenerate before trusting agreement %s in the next tuning cycle. gen-draft-labels HALTS on Sonnet API error + VALIDATES Sonnet output (invalid rows → forced contested, never silent ground truth).

### In-app display (post-flip: LIVE `_inferred` columns; `_preview` twins are VESTIGIAL)
`app/profile/[id]/page.tsx` (per-role card) + `app/components/ProfileDrawer.tsx` (header function/specialty + work-history cards; provenance chip = `people.classifier_version`) + `app/components/ProfileTable.tsx` (Function + Specialty columns) read the REAL `_inferred`/`specialty_inherited` columns via the shared `lib/classification/current-role.ts` (returns `{fn, specs, inheritedSpecs}` — inherited render muted/italic + ⓘ everywhere; `formatAxisLabel` = Title Case + acronyms; ⓘ tooltip UX fix queued in BUGS). The orange "AI CLASSIFICATION" label is scaffolding to restyle at launch. The 089/092 `_preview` columns + `populate-preview.ts` are eval-era scaffolding — no runtime reader remains; drop in a cleanup pass, don't reuse.

### Legacy title-resolution fixes (2026-07-01/07-08, prod)
Both `seniority_normalized` (`lib/normalize/seniority.ts` — both resolvers now scan the TITLE for leadership/level signals after exact-match fails) and `title_level` (`lib/normalize/title-level.ts::extractTitleLevel` — added a leadership fallback) had the same compound-title bug ("Robotics Software Engineering Manager" → IC / unleveled). Backfilled 257 seniority + 88 title_level values. **Priced into scores by the 2026-07-08 post-merge full re-score** (part of why 9 candidates promoted; see CHANGELOG).

### Vocab state — PROD == DEV at hash `83e9c32a` (verified live 2026-07-13)
Both DBs: 17 active functions / **150 active specialties** (090: +computer_vision/nlp/sre/devops/platform; 091: +robotics_software_engineering reactivated-guarded + thermal/reliability parent-widenings; 093: +powertrain_engineering [mechanical, electrical]; 096: 4 prod-only drift rows deactivated — blockchain/game/qa_testing/robotics) / **196 active skills** (192 at the 07-08 merge freeze + 4 from the 07-09 tactical pass: Git, GitHub, Linux, Distributed Systems). The tactical pass moved the vocab hash `33c400c8` → **`83e9c32a`** on both DBs (parity held — the fork was vs the frozen contract, not dev-vs-prod); Step 0 of the 2026-07-13 sequence reclassifies the pre-fork candidates to uniform `83e9c32a` provenance.

**What is IN the hash** (`lib/candidates/classifier/index.ts::loadActiveVocab`, exact): sorted active function names + sorted active specialty names + sorted active skill `canonical_name`s + the specialty→`parent_function[]` map with parent arrays in DB-STORED ELEMENT ORDER (order-sensitive — reordering a parent array forks the hash). **What is NOT in the hash** (an earlier version of this section wrongly claimed role-map membership): `role_specialty_map` (search-side only — edits are hash-safe but change role-pill expansion), skill `aliases` (feed the person-skills matcher only), `primary_specialty` hints, `is_searchable`. Adding/renaming/(de)activating any function, specialty, or skill canonical name forks the hash; the classifier prompt is built dynamically from the live vocab, so vocab changes alter live classifier behavior immediately even with PROMPT_VERSION unchanged.

`sync-reference.mjs` has `--dev` (targets dev; default prod) + a hard gate validating every skills hint against the TARGET DB's active specialties (fails loud, also in --dry-run). ⚠ Any future vocab change must go to BOTH DBs (dev-first) or the hash forks dev-vs-prod. New canonical rows move the hash; alias-only edits don't.

### MERGE ARC — COMPLETE (2026-07-08, PR #16)
All steps ran gated: snapshots (`_mergearc_20260708` schema on prod — drop after stability window) → prod migrations 085–097 → scoped skills sync → full classifier run → flip PR + Matt browser-verify + Codex adversarial review → merge → full re-score. Separate batches remain: `employment_type`/full-time capture cleanup; dup-experiences ingest race (BUGS.md); extension skills-section scrape + person-level alias matcher (BACKLOG); companies-scoring coverage (SeJun-class candidates pinned by unscored employers); PostgREST 14.1→14.5 parity + ingest classify hook + ⓘ tooltip (BUGS.md).
...
### PR 2b — candidate-search integration (PR [#14](https://github.com/mktahr/vetted/pull/14), 2026-06-29)
Enriched connections are projected into `people` (`record_kind='network_connection'`) so the existing 25-axis search machinery is reused, but they are **excluded from the default candidate pool** (enrichment ≠ promotion). Both open decisions resolved: **data tier (ii)** confirmed feasible via a live `/person/enrich` probe (rich data returns only when `fields=[basic_profile,experience,education,skills]` is requested; `certifications`/`honors` are denied on our account), and the **`people`-projection-with-`record_kind`** architecture adopted (a kind column, not a boolean: `candidate` | `network_connection` | `both`; default `candidate` is fail-safe). Pool membership DERIVES from `record_kind`: default pool = `IN (candidate,both)`; search-within-connections = `IN (network_connection,both)`; promotion flips `network_connection`→`both`.

Pieces: enrich `fields` fix; `record_kind`/`person_id` migrations (080/081); server-side pool filters on every broad reader; cache freshness; `mapEnrichToCanonical` (enrich-specific mapper preserving description/employment_type/field_of_study/skills); `writeCanonicalProfile` (transport-agnostic normalize-and-write core extracted from `/api/ingest`, candidate path proven byte-equivalent); `projectConnection` (resolve-existing-person-first → merge or `network_insert`, `record_kind` transition applied LAST, bounded 23505 recovery); search-within-connections scope + org/employee picker.

### PR 2 — gated promotion + cross-org view (branch `network-connections-gated-promotion`, migration 082; in PR / preview verification 2026-06-29)

**Gated promotion** — admin moves a projected connection into the general candidate pool. It is a flag flip — `record_kind` `network_connection`→`both` + the `person_id` link — on top of PR 2b projection; **no re-pay / re-enrich / re-score** (the row was scored at projection time). Two inputs, admin override wins:
- **Auto-rule:** a connection at a **vetted company** (`companies.review_status='vetted'`, joined via the connection's overlay `company_id`) is eligible. The `desiredInPool()` predicate in [lib/network/promote.ts](lib/network/promote.ts) is the single isolated definition of "vetted" — swap it there if the definition changes. (Eligible-but-unenriched can't promote — projection needs a blob; surfaced as "needs enrichment".)
...
Mechanics live in `reconcileConnectionPool()` (one connection → desired state, idempotent) + `reconcileConnections()` (batch). **Demotion safety:** force-out demotes `both`→`network_connection` ONLY when `people.promoted_from_connection=TRUE` *and* no other linked connection (N:1) still wants the person in — a **native candidate is never removed from the pool**. Proven on dev via a seeded rolled-back guard test (promote lifts only `network_connection`; demote fires only on promotion-origin rows). Surfaces: `POST /api/network/promote` (`mode:'auto'|'set'`); Pool column + per-row Auto/Force-in/Force-out control + "Auto-promote vetted" toolbar button on [app/network/connections/page.tsx](app/network/connections/page.tsx); pool state computed in the connections GET.

**Admin cross-org view** — `GET /api/network/cross-org?person_id=…` returns every org + individual connected to a candidate (matched by both the `person_id` link AND `canonical_url`, active owners only; no migration). Rendered by [app/components/CrossOrgNetwork.tsx](app/components/CrossOrgNetwork.tsx) on the candidate profile page (invisible when the candidate has no connections).

**Deferred (BACKLOG):** any additional candidate "bar" beyond the vetted-company check; the candidate-ingest→`both` symmetric-promote edge (clear `promoted_from_connection` when a promoted connection is later candidate-ingested).

---

## Database: Final Schema State (after migrations 001–081; 080–081 prod-applied 2026-06-29, code on PR #14)
...
- 080 — `people.record_kind` (Network Connections PR 2b, PR [#14](https://github.com/mktahr/vetted/pull/14) — **dev + prod-applied 2026-06-29**). `TEXT NOT NULL DEFAULT 'candidate' CHECK IN (candidate, network_connection, both)` + partial index on non-candidate rows. A KIND column, not a boolean (Codex caught that a FALSE-default boolean would silently drop every existing row out of search). DEFAULT `'candidate'` is fail-safe: existing rows + new ingest inserts stay in the pool; only the normalizer writes `network_connection`. Pool membership derives from this column (default pool = `IN (candidate,both)`). Additive; inert until code reads it. See "Network Connections Module → PR 2b".
- 081 — `connections.person_id` (PR [#14](https://github.com/mktahr/vetted/pull/14) — **dev + prod-applied 2026-06-29**). Nullable FK → `people(person_id) ON DELETE SET NULL`, partial index. **N:1, NOT 1:1** — many connections (across orgs) may link to one person; the FK is on the connections side. NULL until the normalizer projects the connection. Promotion = set this link + flip `record_kind`→`both`. ON DELETE SET NULL keeps the connection (+ its enrichment) if the person row is removed. (SQL/DB comment in 081 still reads "1:1" — cosmetic, the real cardinality is N:1.)
- 082 — Network Connections PR 2 **gated promotion** (PR [#15](https://github.com/mktahr/vetted/pull/15) — **merged + dev+prod-applied 2026-06-29**). Two additive columns: `connections.pool_override` (TEXT, CHECK `NULL|in|out` — the admin's explicit per-connection pool decision, final say; persists across auto-rule re-runs) + `people.promoted_from_connection` (BOOLEAN NOT NULL DEFAULT FALSE — provenance guard: TRUE only when pool membership came from promoting a *pure* network connection; force-out demotes ONLY when TRUE so a native candidate is never removed). Both ADD COLUMN on existing tables (no RLS re-enable concern). Promotion is a flag flip — `record_kind` `network_connection`→`both` — on PR 2b projection; **no re-pay/re-enrich/re-score**. Auto-rule gate = vetted company (`companies.review_status='vetted'` via the connection's overlay `company_id`). See "Network Connections Module → PR 2 (gated promotion)".
- 083 — Five-axis classifier engine (sub-PR 3, branch `five-axis-subpr3-classify` — **dev + PROD-applied 2026-07-01**; additive/inert). `person_experiences` += `function_inferred TEXT[]` / `specialty_inferred TEXT[]` / `skills_inferred TEXT[]` / `title_normalized_inferred TEXT`. `people` += classification lifecycle: `classification_status` (pending|in_progress|done|failed — **the live classifier QUEUE key**) / `classification_generation` / `classifier_version` / `classified_at` / `classification_lease_token` / `classification_lease_expires_at` / `classification_failure_count`. New tables `candidate_classification_runs` + `candidate_classification_spend_log`. RPCs: `commit_classification` (atomic + generation-fenced + exact-experience-coverage), `reserve_classification_spend` (atomic cap check), `bump_classification_generation`. See "Five-Axis Classifier (sub-PR 3)".
- 084 — RLS-off follow-up for the 083 tables. Dev + PROD.
- 085/086/087 — All-startup-engineering taxonomy refinement (**DEV-ONLY; prod taxonomy + person-data cascade happen AT MERGE**). 085: add `ai_engineering`/`ml_platform_engineering` (software), `llm_engineering`/`recommendation_ranking_engineering` (ml), `data_pipeline_engineering` (data); deprecate ml_ops/ml_infrastructure/redundant ml_engineering-specialty/data_analytics/analytics_engineering/mechanical_design; rename ai_research→ml_research_engineering; uniform `_engineering` suffix on active eng specialties. 086: deprecate technical_program_management (TPM→unknown), move hardware_description_languages/kinematics/machining/prototyping to the SKILLS axis, merge autonomy/controls/embedded `_software`→`_engineering`, rename metallurgy→metallurgical, de-double-tail flight/ground/mission/perception/simulation, KEEP slam_engineering. 087: **deprecate `founder` as a FUNCTION + its 4 specialties** (ceo/co_founder/founding_engineer/founding_team_member) — founders live on `is_current/former_founder` booleans, NOT the function axis; founding engineers route to their real discipline. Post-087 dev vocab: 17 active functions / 143 active specialties.
- 088 — Founding/early-ENGINEER searchable TAG (**dev + PROD**). `person_experiences.is_founding_engineer_role` + `people.has_founding_engineer_experience` (+ partial index). Title-driven regex in `compute-derived.ts::FOUNDING_ENGINEER_TITLE_PATTERN` ("Founding [X] Engineer" / "First [X] Engineer" / "Early Engineer" / "Engineer #N"; 26 fixture tests in `scripts/eval/test-founding-regex.ts`). Distinct from the founder flags. Shaped so the roadmap headcount/founding-date inference layer can populate it later.
- 089 — Classification **PREVIEW** columns (**dev + PROD**; additive/inert). `person_experiences` += `function_inferred_preview` / `specialty_inferred_preview` / `skills_inferred_preview` / `title_normalized_inferred_preview` + provenance `classification_preview_version` / `classification_preview_at`. SEPARATE from the real `_inferred` columns and from `classification_status` (the live queue key) — a preview writer touches ONLY these, never lifecycle. Populated by `scripts/eval/populate-preview.ts` for in-app review; the real classifier populates the real `_inferred` columns via `commit_classification` post-merge.
- 090 — Specialty gap-fill (**DEV-ONLY; joins 085–087 at merge**). +5 active specialties evidenced by the 2026-07-05 vocab gap analysis: `computer_vision_engineering` + `nlp_engineering` (parent `ml_engineering`), `sre_engineering` + `devops_engineering` + `platform_engineering` (parent `software_engineering`). Fixes the PROMPT-VOCAB DRIFT (classifier prompt Rule 5 already instructed sre/devops/platform values that didn't exist — evidenced misfile: "Senior SRE - Infrastructure" → `reliability_engineering`). Dev: 148 active specialties. Empty-DB-tolerant verification block.
- 091 — Robotics carve-out taxonomy + parent-widenings (**DEV-ONLY; joins 085–090 at merge**). REACTIVATES `robotics_software_engineering` (killed unguarded in 086) under a guarded definition — the "software engineer on the robot stack" archetype, single parent `robotics_engineering`; sharper robotics specialties always win. Pairs with the cls-2026-07-08a Rule-5 robotics carve-out (title modifier = BUILD-target → robotics; industry/context → software; employer name never triggers — enforced deterministically by `carve-out-guard.ts`). Also widens `thermal_engineering` += aerospace_engineering, `reliability_engineering` += electrical_engineering (append-only, idempotent).
- 092 — Deterministic career-fallback columns + `commit_classification` extension (**dev + PROD-applied 2026-07-07**; additive/inert). `person_experiences` += `specialty_inherited TEXT[]` (code-computed inheritance from `computeCareerFallback` — `specialty_inferred` stays evidenced-only LLM output; search reads the union) + `specialty_inherited_preview TEXT[]` (preview twin, 089 isolation contract). The commit RPC writes `specialty_inherited` atomically with the evidenced axes (backward compatible — missing key → NULL). See "Five-Axis Classifier → DETERMINISTIC layers".
- 093 — `powertrain_engineering` specialty (**dev + PROD-applied at merge 2026-07-08**). Parents `[mechanical_engineering, electrical_engineering]` (genuine EV-era cross). SeJun strip-review resolution: an all-powertrain career was inexpressible — the model reached for `motor_drives_engineering` (motor-CONTROL electronics, a different discipline), the parent guard correctly stripped it, the evidence bar correctly left empty. Correct strip, missing row. (085/086/087/090/091 likewise promoted to prod in the 2026-07-08 merge arc.)
- 094 — `role_specialty_map` remap to the post-085–093 vocabulary (**dev + PROD, merge arc 2026-07-08**). The map has NO FK on `specialty_normalized`, so the dictionary renames never cascaded. 12 explicit renames (`ai_research`→`ml_research_engineering`, the `_software` de-tails/merges, `metallurgy`, `robotics_software`, `mechanical_design`) + generic `_engineering` suffix rule + INSERTs for the 8 net-new specialties (ML/software cluster → Software Engineer role per the `applied_ml` precedent; `powertrain_engineering` → Mechanical primary + Electrical secondary) + DELETE of rows with no active-specialty target. Fail-loud invariant: every map row references an ACTIVE specialty. Prod result: 246 → 167 rows.
- 095 — founder-function person-data cascade (**dev + PROD, merge arc**). `function_normalized='founder'` → `'unknown'` across `title_dictionary` (8 rows — without this, ingest re-creates founder rows via `normalizeTitle`), `person_experiences` (12), and a defensive `people.current_function_normalized` recompute (073's ordering). Founder-as-attribute signals (`is_current/former_founder`, seniority `founder` level, former_founder bonus) untouched by design.
- 096 — prod-vocab dev-parity (**dev no-op + PROD, merge arc**). A read-only pre-run simulation showed prod would land at 154 active specialties, not the frozen 150: 4 prod-only active rows absent from dev's curated set (`robotics`→`robotics_engineering` — a FUNCTION-name collision — plus blockchain/qa_testing/game_engineering). Deactivates them (both name forms) + deletes their surviving map rows + re-asserts 094's invariant. NOTE: the frozen vocab deliberately contains 10 self-parented same-name generalist specialties (e.g. specialty `mechanical_engineering` under function `mechanical_engineering`) — a blanket "no specialty may share a function name" check is WRONG; 096's first draft tripped on exactly that on dev.
- 097 — `claim_classification` RPC (**dev + PROD, merge arc**). Moves the classifier's conditional claim (pending | failed<max | expired-lease → in_progress + lease, RETURNING generation + failure count) into SQL. Root cause: **prod PostgREST 14.1 mishandles `PATCH + or=`** — 42703 with a named returning list; with `select=*` it EXECUTES the update but returns an EMPTY representation (first prod run leased 50 people while reporting all not_eligible). Dev (14.5) is unaffected — why every dev test passed. SECURITY DEFINER, service-role-only grants; after psql DDL, PostgREST needs `NOTIFY pgrst, 'reload schema'` or the first call 404s.
...
│   └── ingest/
│       ├── index.ts                             ← barrel
│       ├── write-canonical.ts                   ← writeCanonicalProfile() — transport-agnostic normalize+write CORE (ingest steps 2-9); shared ingest types; opts.identity candidate|network_insert (PR #14)
...
│       └── mappers/
│           ├── crust-v2.ts                      ← mapPersonSearchToCanonical() — LIVE search path; exports shared date/grad/years helpers
│           ├── crust-enrich.ts                  ← mapEnrichToCanonical() — /person/enrich blob → canonical, preserves description/employment_type/field_of_study/skills (PR #14)
...
### ALWAYS provide the URL — proactively, unprompted

Any time Matt needs to open something in a browser — to test, review, or double-check any work — provide the exact clickable URL in the same message, without being asked. This applies to EVERY surface: prod, Vercel previews, localhost, specific pages/routes. Never say "check the preview" or "the app now shows X" without the URL. Making Matt ask "what's the URL?" is a process violation and a waste of a round-trip.

Known URLs (derive, don't rediscover via CLI):
- **Prod:** https://vetted-self.vercel.app
- **Branch preview (stable alias, always latest deploy of the branch):** `https://vetted-git-<branch-slug>-matt-tahrtechs-projects.vercel.app` (slug = branch name lowercased, non-alphanumerics → hyphens; e.g. branch `five-axis-subpr3-classify` → https://vetted-git-five-axis-subpr3-classify-matt-tahrtechs-projects.vercel.app)
- **Local dev:** http://localhost:3000 (`npm run dev`)
- Deep-link when relevant: `/profile/<person_id>`, `/admin/companies`, `/network/connections`, etc.
...
### Dev≠prod is a BUG CLASS, not just a schema concern (2026-07-08 merge-arc lessons)

Two production incidents in one arc, same root:
1. **Vocab drift**: prod's `specialty_dictionary` had 4 active rows dev never had — a pre-run READ-ONLY SIMULATION of the migrations against prod's actual rows caught it (prod would have landed 154≠150 and silently broken the frozen classifier-vocab hash). **Rule: before a prod migration sequence whose end-state was only ever verified on dev, simulate against prod's real rows and diff the projected end-state.**
2. **Infra drift**: prod runs PostgREST **14.1**, dev **14.5**. 14.1 mishandles `PATCH + or=` logical filters — worst case it EXECUTES the update but returns an empty representation (supabase-js sees `data=[]` and reports nothing happened). **Rules: conditional mutations (UPDATE-where-eligible-RETURNING patterns) go through SQL RPCs, never supabase-js `.update().or()`; anything that only ever ran against dev is UNTESTED on prod; keep the two projects' infra versions matched (upgrade queued in BUGS).** Also: after applying DDL via psql, PostgREST may need `NOTIFY pgrst, 'reload schema'` before REST sees new functions.
