...
**Still NOT built (next steps):** **gated promotion** (admin flips `network_connection`→pool, gated on vetted-company + bar — flag flip, no re-pay) and **admin cross-org view** (every org+individual connected to a candidate — schema supports via `canonical_url` + `connection_owners`). See ROADMAP + SESSION_HANDOFF.
