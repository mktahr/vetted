...
export async function GET(req: Request) {
  ...
  let q = supabase
    .from('connections')
    .select('connection_id, full_name, current_company, current_title, title_bucket, title_bucket_source, status, specialty_normalized, company_id, company_score, company_score_year, enriched, last_enriched_at, llm_triage_guess, llm_triage_reason, connected_on, canonical_url, raw_url')
    .eq('org_id', orgId)
  ...
  const rows = (conns ?? [])
    .map((c) => {
      const owners = ownersByConn.get((c as any).connection_id) ?? [];
      return { ...c, owners };
    })
    ...
    .filter((c) => !employeeId || c.owners.some((o) => o.employee_id === employeeId));
