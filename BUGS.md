...
- **Dev/prod schema drift on `specialty_dictionary.function_normalized`.** Surfaced 2026-06-24 during the network-connections build. PROD's `specialty_dictionary` has a `function_normalized TEXT` column (confirmed via `information_schema` query), but **no migration in `supabase/migrations/` creates it** — so a fresh dev DB replayed from migrations lacks it. App code reads it at [lib/normalize/specialty.ts:56](lib/normalize/specialty.ts#L56) (`select('specialty_normalized, function_normalized, …')`). On **prod this works** (column present → `/api/ingest` specialty resolution is fine); on **dev it throws** `column specialty_dictionary.function_normalized does not exist`, breaking `loadSpecialtyDictionary()` and anything that calls it on dev. Pure parity gap — prod is healthy; this is NOT the cause of the `parent_function` render crash (that was fixed on `hotfix-parent-function-array-render`). Fix options: (a) add `ALTER TABLE specialty_dictionary ADD COLUMN IF NOT EXISTS function_normalized TEXT` as a migration so dev replays match prod, or (b) decide the column is redundant post-072 (its info now lives in `parent_function[]`) and drop it from both the table and the resolver's `select`. ~0.5 day.
