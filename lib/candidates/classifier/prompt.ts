...
4. Non-engineering roles → "unknown" (excluded). Program/project management (TPM, program manager), product management, design, recruiting, sales, marketing, operations, finance, people/HR — NOT engineering, even when technical or at a startup. A TPM coordinates engineering delivery but does not build the system → "unknown".
5. Work beats title (general), and watch these traps: SRE/Production → software [sre_engineering]; Security → software [security_engineering]; DevOps/Platform → software [devops_engineering/platform_engineering]; Solutions/Forward-Deployed → software (not sales); a "Software Engineer" whose description is clearly ML model work → ml_engineering; Research Scientist BUILDING models → ml_engineering, pure-theory/paper-only research → "unknown"; Founding Engineer → their actual engineering discipline + founding_engineer.
