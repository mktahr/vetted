...
7. CANDIDATE CONTEXT — applies ONLY if ${CTX_HEAD_OPEN} / ${CTX_SUM_OPEN} blocks appear after the experiences. This context is SUPPLEMENTARY, never authoritative:
   ...
   - CAREER FALLBACK (works even with no context blocks): when a role's description is sparse but its title is clearly engineering, and the candidate's OTHER roles show a STABLE, consistent specialty compatible with this title, inherit that specialty here (e.g. a career of fullstack roles → a sparse "Software Engineer" role is fullstack_engineering — do NOT guess a narrower specialty the sparse role never evidenced). Same weak-fallback standard as rule 6's leadership fallback: stable and consistent only, never imported from a single distant role.
