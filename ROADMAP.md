...
**Active thread → Network Connections PR 2** (candidate-search integration + admin cross-org view + gated promotion). PR 1 pipeline + 2a detail drawer shipped to prod 2026-06-24 ([#10](https://github.com/mktahr/vetted/pull/10), [#13](https://github.com/mktahr/vetted/pull/13)). PR 2 starts with a **Claude+Codex architecture pressure-test** of the data-tier decision (snapshot-axes vs paid rich-enrichment) + the `people`-projection-with-pool-flag model — *before any building*. Full scope in BACKLOG "Network Connections".
...
2. **Five-axis candidate taxonomy rebuild**
   ...
   **Build order (seven sub-PRs, each independent — do not bundle):**
   ...
   2. Dictionary work — split into two sub-PRs:
      ...
      - **2b** ✅ — `function_dictionary` expansion to 16 sub-functions + `specialty_dictionary` cleanup with multi-parent reparenting + V1 scope cut (migrations 071/072/073/074, [PR #9](https://github.com/mktahr/vetted/pull/9), merged + prod-applied 2026-06-21, rescored).
   3. **← NEXT.** Ingest-side LLM-assisted per-experience inference — outputs the five-axis tuple including `title_normalized`. Constrained to controlled vocabulary (active function_dictionary + active specialty_dictionary + active skills_dictionary). Precedent: existing `lib/companies/tagger/` pattern.
...
| 2026-06-24 | Specialty resolver dev/prod parity fix + orphan-column drop | [#12](https://github.com/mktahr/vetted/pull/12) | B-lite: `loadSpecialtyDictionary()` reads `parent_function` (single-parent → scalar; multi → null, defer to sub-PR 3) instead of the orphan `function_normalized` column. Migration 079 drops that column from prod (`DROP COLUMN IF EXISTS`, no-op on dev). Fixes the dev-only `/api/ingest` 400 + stale-umbrella prod ingest. Lockstep held (code→DB). Prod+dev converge on the migration-defined schema. Also: PR #11 (parent_function render hotfix) confirmed merged; session-protocol rename; SESSION_HANDOFF reconstructed; `.claude/settings.local.json` approvals; `block` command + full-copyable-prompt rule added to docs. |
| 2026-06-24 | parent_function TEXT[] render hotfix (prod-down) | [#11](https://github.com/mktahr/vetted/pull/11) | Migration 072 made `specialty_dictionary.parent_function` a TEXT[]; ProfileTable + search-builder still called `.replace()` on it, crashing Candidates + search-builder. Fix: `Array.isArray` guard + `join`. Client-render only, no migration. Verified live on prod. |
| 2026-06-21 | Five-axis taxonomy sub-PR 2b — function/specialty rebuild SHIPPED | [#9](https://github.com/mktahr/vetted/pull/9) | Migrations 071–074 merged + promoted to prod in order (code led DB — lockstep held), each independently verified: 071 function_dictionary 18/18/36; 072 specialty multi-parent 225/166/59/45 (`parent_function` → TEXT[]); 073 person/people reclass (0 orphans, 6 → data_engineering); 074 title remap (7 leadership@eng / 28 SWE / 1 data_eng). Full prod rescore via `/api/admin/rescore-all` (84/84, distribution unchanged vetted 49 / needs_review 35). Next: sub-PR 3 (LLM ingest inference). |
